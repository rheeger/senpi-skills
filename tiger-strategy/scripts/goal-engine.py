...
def recalculate_goal(config, state):
    ...
    # v3: Use margin_pct_per_slot (default 30%) — learned lesson from PUMP sizing mistake
    # Old method (balance÷slots÷leverage) dramatically undersized positions
    # Correct: each slot gets margin_pct% of balance as MARGIN, then leverage multiplies to notional
    ...
    # Per-slot budget: margin_pct_per_slot of balance (as MARGIN, not notional)
    max_slots = config["max_slots"]
    margin_pct = config.get("margin_pct_per_slot", 0.30)
    per_slot = current_balance * margin_pct  # This IS the margin amount
