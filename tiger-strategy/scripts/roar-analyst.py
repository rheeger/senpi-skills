...
from tiger_config import load_config, save_config, load_state, TRADE_LOG_FILE, load_trade_log as _load_trade_log, get_trade_log_path
...
def load_trade_log() -> list:
    """Load the trade log. Delegates to tiger_config.load_trade_log."""
...
def build_scorecard(trades: list, roar_state: dict) -> dict:
    ...
    for t in trades:
        ...
        # Fall back to net_pnl dollar amount if no pct available
        if pnl == 0:
            pnl = t.get("net_pnl", t.get("rpnl", 0.0)) or 0.0
        p["total_pnl_pct"] += pnl
        if pnl > 0:
            p["wins"] += 1
        else:
