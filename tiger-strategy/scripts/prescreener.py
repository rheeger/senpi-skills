...
BLACKLIST = {"PUMP", "kPEPE", "kSHIB"}  # kPEPE/kSHIB: Senpi API uppercases coin names → "Invalid coin"
TOP_N = 30
...
def main():
    ...
    # xyz: assets can't fetch candles from MCP — filter them out
    # xyz: assets now supported via synthetic candles
    ...
    # CRITICAL: Separate xyz: assets from main dex assets.
    # xyz: must NEVER contaminate main dex scan groups.
    # A hanging xyz: fetch must never block BTC/ETH/SOL scanning.
    main_candidates = [c for c in candidates if not c["name"].startswith("xyz:")]
    xyz_candidates = [c for c in candidates if c["name"].startswith("xyz:")]

    # Split MAIN dex into 6 groups of 5 by volume (a=highest, f=lowest)
    by_vol = sorted(main_candidates, key=lambda x: x["day_ntl_vlm"], reverse=True)
    chunk = 5
    groups = {}
    group_labels = ["a", "b", "c", "d", "e", "f"]
    for i, label in enumerate(group_labels):
        start = i * chunk
        groups[f"group_{label}"] = [c["name"] for c in by_vol[start:start + chunk]]

    # xyz: gets its own isolated group — scanned separately, failures can't cascade
    if xyz_candidates:
        xyz_by_vol = sorted(xyz_candidates, key=lambda x: x["day_ntl_vlm"], reverse=True)
        groups["group_xyz"] = [c["name"] for c in xyz_by_vol[:5]]
    ...
    # Write prescreened.json
    atomic_write(PRESCREENED_FILE, result, indent=2)
